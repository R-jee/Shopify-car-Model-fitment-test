...
$("#test_fitment_id").on('click', function () {
    if ($("#fitment_sku_id").val() != "" && $("#fitment_year_id").val() != "" && $("#fitment_model_id").val() != "") {
        // console.log("Pass...");
        $.ajax({
            url: "https://console.autooutletllc.com/miniapp_shopify/fitment_test_process.php",
            type: "post",
            data: {
...
$('#bundle-trigger___id').on("click", function () {
    ...
    for (let sku_i = 0; sku_i < leng; sku_i++) {
        ...
        if(temp_sku != ""){
            $.ajax({
                url: "https://console.autooutletllc.com/miniapp_shopify/fitment_getData_with_sku.php",
                type: "POST",
                async: false,
                data: {
                    'getData_with_sku': 1,
...
$( document ).ready(function() {
  ...
  $.ajax({
    url: "https://console.autooutletllc.com/miniapp_shopify/fitment_dataTable_process.php",
    type: "post",
    async: false,
    data: {
      sku___: temp___sku__,
